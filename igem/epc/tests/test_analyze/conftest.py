...
def data_NHANES_lonely():
    df = igem.load.from_csv(DATA_PATH / "nhanes_lonely_data.csv", index_col=None)  # noqa E501
    df = igem.modify.make_binary(df, only=["HI_CHOL", "RIAGENDR"])
    df = igem.modify.make_categorical(df, only=["race", "agecat"])
    return df
...
def genotype_case_control_add_add_main():
    var1 = scalars.Variant(chromosome="1", position=123456, id="rs1", ref="T", alt="A")  # noqa E501
    var2 = scalars.Variant(chromosome="10", position=30123, id="rs2", ref="A", alt="C")  # noqa E501
